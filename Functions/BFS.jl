    ...
    while opf_err >= 1e-4 && opf_iter<max_iter && (stat == "LOCALLY_SOLVED" || stat == "ALMOST_LOCALLY_SOLVED")
        ...
        elseif opf_iter>5 && err_vec[opf_iter-1] >10
            ...
            for i=1:nbr
                ...
                jj = Int.(jjjj); kk = Int.(kkkk); Zbr = -pinv(Y[jj,kk]); 
                Ibr_d = (Sbus_d[kk].*cos.(θ0[kk]) + Sbus_q[kk].*sin.(θ0[kk]))./Vm0[kk];
                Ibr_q = (Sbus_d[kk].*sin.(θ0[kk]) - Sbus_q[kk].*cos.(θ0[kk]))./Vm0[kk];
                ...
                if tf_branch[Int(line_prove[i,4]),3] == 8
                    ...
                    @constraint(m, Vd[kk] .== vr_tap*(Vd[jj] - real(Zbr)*Ibr_d + imag(Zbr)*Ibr_q) )
                    @constraint(m, Vq[kk] .== vr_tap*(Vq[jj] - real(Zbr)*Ibr_q - imag(Zbr)*Ibr_d) )
                   count2 = count2 + 1;
                else
                    @constraint(m, Vd[kk] .== Vd[jj] - real(Zbr)*Ibr_d + imag(Zbr)*Ibr_q )
                    @constraint(m, Vq[kk] .== Vq[jj] - real(Zbr)*Ibr_q - imag(Zbr)*Ibr_d )
                end
                # Vm_jj = Vm1[jj].*exp.(im*θ1[jj]); ii = Int(line_prove[i,4])
                # if bus_ph[k] == "AN" || bus_ph[k] == "AS" || bus_ph[k] == "A"
                #     Vm_kk = A_mat[1,1,ii]*Vm_jj - B_mat[1,1,ii]*Ibr[count1:count1+length(kk)-1]
