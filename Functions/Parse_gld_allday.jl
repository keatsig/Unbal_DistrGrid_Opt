...
function parse_gld(crit_node)
    ...
    a_mat = zeros(3,3)*(1+0*im); b_mat = zeros(3,3)*(1+0*im);
    for i=1:nbr
    ...
    yinv_powerP = PV_prof[:,count_sch];
    global PV_pen = zeros(n_sch+1); PV_pen[1] = sum(yinv_powerP)/tot_load*100;
    ...
    @show par_det
    print("-------------------\n")
    min_ivl = 60/(n_sch/24); min_str = 0:min_ivl:59;
    min_str = string.(Int.(min_str)); global mins = repeat(min_str,n_sch)
    hr = 0:1:23; global hrs = zeros(length(mins)); hr_ivl = Int(n_sch/24);
    for i=1:length(hr)
        hrs[i*hr_ivl-(hr_ivl-1):i*hr_ivl] = repeat([hr[i]],hr_ivl)
    end
    hrs = string.(Int.(hrs));  mins = mins[1:end]
end
